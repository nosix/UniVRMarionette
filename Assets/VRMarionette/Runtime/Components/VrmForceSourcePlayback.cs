...
{
    /// <summary>
    /// VrmForceSource の trace ログから動きを再現する。
    /// </summary>
    [RequireComponent(typeof(VrmForceSource))]
    public class VrmForceSourcePlayback : MonoBehaviour
    {
        ...
        private VrmForceSource _controlTarget;
        private readonly Queue<Command> _commands = new();

        private static readonly Regex TraceLog = new(@"VrmForceSource (\w+) \(([^)]+)\) \(([^)]+)\) (\w+)");

        public void OnModelReady()
        {
            _controlTarget = GetComponent<VrmForceSource>();
        }
