...
{
    /// <summary>
    /// Unity Editor 上で Humanoid を操作するためのコンポーネント。
    /// HumanoidMixerEditor と連携して使われる。
    /// HumanoidManipulator コンポーネントを持つ GameObject に追加して使用する。
    /// </summary>
    public class HumanoidMixer : MonoBehaviour
    {
        public HumanoidManipulator manipulator;
        ...
        private void Start()
        {
            manipulator = GetComponent<HumanoidManipulator>();
            if (manipulator is null)
            {
                Debug.LogError("Please set to the GameObject with the HumanoidManipulator Component.");
            }
        ...
        {
            if (manipulator) manipulator.SetBoneRotation(bone, angle);
        }
